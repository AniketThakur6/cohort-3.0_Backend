...
const ResultCard = ({ url, openQrId, setOpenQrId }) => {
    ...
    <div className="flex ml-1 items-center h-15 bg-zinc-900 border-2 border-gray-500 rounded-lg  w-280 px-3">
      ...
      <div className="flex-1 ml-2 flex gap-6">
        <a
          ...
          target="_blank"
          className="flex text-amber-500 font-medium hover:underline transition-all "
        >
          {url?.shortCode}
        </a>
        <p className="w-88 text-gray-400 truncate">{url?.originalUrl}</p>
      </div>
      <div className="flex gap-3">
        ...
        <button
          onClick={handleCopy}
          className="flex cursor-pointer items-center gap-1 px-3 py-0 rounded bg-amber-600 hover:bg-amber-700 transition-colors duration-250"
        >
          {copy ? (
            <>
